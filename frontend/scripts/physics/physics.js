class Universe {
  ...
  static calculateCollide(objectA, objectB) {
    ...
    newObject.setPositionX(
      (objectA.x * objectA.mass + objectB.x * objectB.mass) / newObject.mass
    );
    newObject.setPositionY(
      (objectA.y * objectA.mass + objectB.y * objectB.mass) / newObject.mass
    );

    newObject.setVelocityX(
      (objectA.mass * objectA.vx + objectB.mass * objectB.vx) / newObject.mass
    );
    newObject.setVelocityY(
      (objectA.mass * objectA.vy + objectB.mass * objectB.vy) / newObject.mass
    );
    if (
      _this.objectSelected &&
      (_this.objectSelected.id == objectA.id ||
        _this.objectSelected.id == objectB.id)
    ) {
      _this.objectSelected = newObject;
    }
    if (
      _this.objectCentered &&
      (_this.objectCentered.id == objectA.id ||
        _this.objectCentered.id == objectB.id)
    ) {
  ...
  static calculateGravity() {
    ...
    for (let i = 0; i < Universe.objectsToListen.length; i++) {
      for (let j = 0; j < Universe.objectsToListen.length; j++) {
        if (i !== j) {
          if (
            (Universe.objectsToListen[i].hasGravity ||
              Universe.objectsToListen[j].hasGravity) &&
            ((Universe.objectsToListen[i].x <= Universe.objectsToListen[j].x &&
              Universe.objectsToListen[i].x +
                Universe.objectsToListen[i].vx +
                Universe.objectsToListen[i].radius >
                Universe.objectsToListen[j].x +
                  Universe.objectsToListen[j].vx -
                  Universe.objectsToListen[j].radius) ||
              (Universe.objectsToListen[i].x > Universe.objectsToListen[j].x &&
                Universe.objectsToListen[i].x +
                  Universe.objectsToListen[i].vx -
                  Universe.objectsToListen[i].radius <
            ...
            ((Universe.objectsToListen[i].y <= Universe.objectsToListen[j].y &&
              Universe.objectsToListen[i].y +
                Universe.objectsToListen[i].vy +
                Universe.objectsToListen[i].radius >
                Universe.objectsToListen[j].y +
                  Universe.objectsToListen[j].vy -
                  Universe.objectsToListen[j].radius) ||
              (Universe.objectsToListen[i].y > Universe.objectsToListen[j].y &&
                Universe.objectsToListen[i].y +
                  Universe.objectsToListen[i].vy -
            ...
            Universe.calculateCollide(
              Universe.objectsToListen[i],
              Universe.objectsToListen[j]
            );
            j--;
            i--;
            break;
    ...
    for (let i = 0; i < Universe.objectsToListen.length; i++) {
      ...
      for (let j = 0; j < Universe.objectsToListen.length; j++) {
        if (i !== j && Universe.objectsToListen[j].hasGravity) {
          ...
          accelerationX +=
            (acceleration *
              (Universe.objectsToListen[j].x - Universe.objectsToListen[i].x)) /
            distanceSq;
          accelerationY +=
            (acceleration *
              (Universe.objectsToListen[j].y - Universe.objectsToListen[i].y)) /
            distanceSq;
        }
      }
